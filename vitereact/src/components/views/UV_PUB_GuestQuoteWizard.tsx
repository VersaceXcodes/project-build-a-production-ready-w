...
const UV_PUB_GuestQuoteWizard: React.FC = () => {
  ...
  // Local state - 7 steps for guest wizard
  ...
  const [stepValidation, setStepValidation] = useState({
    step_1_valid: false,
    step_2_valid: false,
    step_3_valid: true, // Files are optional
    step_4_valid: false,
    step_5_valid: true, // Summary is always valid
    step_6_valid: false,
  });
  ...
  const [step6FieldErrors, setStep6FieldErrors] = useState<Record<string, string>>({});
  ...
  // Guest file upload mutation (stores locally for now, attached on submission)
  const uploadFileMutation = useMutation({
    mutationFn: async (file: File) => {
      // For guest uploads, we'll create a local preview first
      // and store the file for later upload when quote is submitted
      return new Promise<UploadedFile>((resolve) => {
        ...
        reader.onload = () => {
          const fileData: UploadedFile = {
            ...
            dpi_warning: false // We'll check DPI on server side
          };
          // Simulate progress
          let progress = 0;
    ...
    onSuccess: (upload: UploadedFile) => {
      ...
      setNotification({
        type: 'success',
        message: `File "${upload.file_name}" ready for upload`
      });
  ...
  const submitQuoteMutation = useMutation({
    ...
    onSuccess: (response) => {
      // Clear wizard data
      localStorage.removeItem('guest_quote_wizard_draft');
      
      setNotification({
        type: 'success',
        message: "Quote submitted! Check your email for confirmation."
      });
      
      setTimeout(() => {
        navigate(`/quote/guest/confirmation/${response.quote.id}`);
      }, 1500);
    },
    onError: (error: any) => {
      setNotification({
  ...
  useEffect(() => {
    const savedDraft = localStorage.getItem('guest_quote_wizard_draft');
    if (savedDraft) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('guest_quote_wizard_draft', JSON.stringify(draftData));
  }, [selectedService, serviceOptions, projectDetails, uploadedFiles, selectedTier, guestDetails, stepValidation]);
  ...
  // Validate Step 6 (Guest Details) whenever it changes
  useEffect(() => {
    const isValid = guestDetails.guest_name.trim() !== '' && 
                    guestDetails.guest_email.trim() !== '' &&
                    /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(guestDetails.guest_email);
    setStepValidation(prev => ({ ...prev, step_6_valid: isValid }));
  }, [guestDetails]);
  ...
  const handleGuestDetailChange = (key: keyof GuestDetails, value: string) => {
    setGuestDetails(prev => ({ ...prev, [key]: value }));
    setStep6FieldErrors(prev => ({ ...prev, [key]: '' }));
  };
  ...
  const goToStep = (step: number) => {
    ...
    if (step > currentStep) {
      ...
      if (currentStep === 6 && !stepValidation.step_6_valid) {
        // Validate specific fields
        const errors: Record<string, string> = {};
        if (!guestDetails.guest_name.trim()) errors.guest_name = 'Name is required';
        if (!guestDetails.guest_email.trim()) errors.guest_email = 'Email is required';
        else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(guestDetails.guest_email)) errors.guest_email = 'Invalid email format';
        setStep6FieldErrors(errors);
  ...
  const handleSubmitQuote = () => {
    ...
    // Final validation
    if (!guestDetails.guest_name.trim() || !guestDetails.guest_email.trim()) {
      setNotification({
        type: 'error',
    ...
    const quoteData = {
      ...
      guest_email: guestDetails.guest_email.trim().toLowerCase(),
      guest_phone: guestDetails.guest_phone.trim() || null,
      guest_company_name: guestDetails.guest_company_name.trim() || null
    };
  ...
  const stepLabels = ['Service', 'Details', 'Files', 'Tier', 'Summary', 'Your Info', 'Submit'];
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Header */}
          <div className="mb-8">
            <Link to="/quote/start" className="text-sm text-gray-600 hover:text-gray-900 mb-4 inline-flex items-center">
              <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
              Back to Quote Options
            </Link>
            <h1 className="text-3xl font-bold text-gray-900 mt-2">Get a Quote</h1>
            <p className="text-gray-600 mt-1">
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 mr-2">
                Guest Mode
              </span>
              No account required - Step {currentStep} of 7
            </p>
          </div>
          ...
          <div className="mb-8 bg-white rounded-lg p-4 sm:p-6 shadow-sm">
            ...
            <div className="sm:hidden mb-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wide">Step {currentStep} of 7</p>
                  <p className="text-sm font-semibold text-gray-900">{stepLabels[currentStep - 1]}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-yellow-600">{Math.round((currentStep / 7) * 100)}%</p>
              </div>
            </div>
            ...
            <div className="hidden sm:flex items-center justify-between mb-4">
              {[1, 2, 3, 4, 5, 6, 7].map((step) => (
                <div key={step} className="flex flex-col items-center cursor-pointer" onClick={() => step < currentStep && goToStep(step)}>
                  <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center font-semibold text-sm transition-all ${
                  ...
                  <span className="text-xs text-gray-600 mt-2 hidden lg:block">{stepLabels[step - 1]}</span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 sm:p-8">
                ...
                <div className="space-y-6">
                  {serviceOptions.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      <p>No configuration options for this service.</p>
            ...
            {currentStep === 3 && (
              <>
                ...
                <p className="text-gray-600 mb-6">Upload your design files or reference materials (optional)</p>
                      ...
                      <div key={file.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div className="flex items-center space-x-3 flex-1 min-w-0">
                          {file.file_type.startsWith('image/') ? (
                            <img src={file.file_url} alt={file.file_name} className="w-10 h-10 object-cover rounded" />
                          ) : (
                ...
                <p className="text-sm text-gray-500 mt-4 text-center">
                  Files are optional. You can also send files later via email.
                </p>
              </>
            )}
            ...
            {currentStep === 5 && selectedService && selectedTier && (
              <>
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Review Your Quote</h2>
                <p className="text-gray-600 mb-6">Please review your selections before providing your contact details</p>
                ...
                <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600">{uploadedFiles.length} file(s) ready</p>
                  {uploadedFiles.length > 0 && (
                ...
                <div className="mb-6 p-6 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-xs text-blue-700 mt-2">Final pricing will be confirmed after review</p>
                </div>

                {/* Timeline Disclaimer */}
                <div className="mb-6 p-4 bg-gray-100 rounded-lg">
                  <p className="text-xs text-gray-600">
                    This is an estimate based on the information provided. Our team will review your requirements and provide a final quote within 24 hours. Turnaround times vary by tier and project complexity.
                  </p>
                </div>
              </>
            )}
            ...
            {currentStep === 6 && (
              <>
                ...
                <p className="text-gray-600 mb-6">We'll use this to send your quote and updates</p>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleGuestDetailChange('guest_name', e.target.value)}
                      className={`w-full px-4 py-3 border-2 rounded-lg focus:ring-4 focus:ring-blue-100 outline-none transition-all ${
                        step6FieldErrors.guest_name ? 'border-red-500' : 'border-gray-200 focus:border-blue-500'
                      }`}
                      placeholder="Enter your full name"
                    />
                    {step6FieldErrors.guest_name && (
                      <p className="text-sm text-red-600 mt-1">{step6FieldErrors.guest_name}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleGuestDetailChange('guest_email', e.target.value)}
                      className={`w-full px-4 py-3 border-2 rounded-lg focus:ring-4 focus:ring-blue-100 outline-none transition-all ${
                        step6FieldErrors.guest_email ? 'border-red-500' : 'border-gray-200 focus:border-blue-500'
                      }`}
                      placeholder="your.email@example.com"
                    />
                    {step6FieldErrors.guest_email && (
                      <p className="text-sm text-red-600 mt-1">{step6FieldErrors.guest_email}</p>
                    )}
                    <p className="text-xs text-gray-500 mt-1">We'll send your quote confirmation here</p>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleGuestDetailChange('guest_phone', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={guestDetails.guest_company_name}
                      onChange={(e) => handleGuestDetailChange('guest_company_name', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all"
                ...
                <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                  <div className="flex items-start">
                    ...
                    <div>
                      <h4 className="font-medium text-green-900">No account required</h4>
                      <p className="text-sm text-green-700">We won't create an account for you. You'll receive a secure link to view and manage your quote.</p>
                    </div>
                  </div>
                </div>
              </>
            ...
            {/* STEP 7: Final Submit */}
            {currentStep === 7 && selectedService && selectedTier && (
              <>
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Submit Your Quote Request</h2>
                <p className="text-gray-600 mb-6">Review everything one last time and submit</p>
                ...
                <div className="border-2 border-gray-200 rounded-xl p-6 mb-6">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">{guestDetails.guest_name}</p>
                      <p className="text-sm text-gray-600">{guestDetails.guest_email}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Files</p>
                      <p className="font-semibold text-gray-900">{uploadedFiles.length} file(s)</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-6 p-6 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <h3 className="font-semibold text-yellow-900 mb-2">Estimated Price Range</h3>
                  {estimateSubtotal ? (
                    <p className="text-3xl font-bold text-yellow-900">${estimateSubtotal.toFixed(2)} - ${(estimateSubtotal * 1.2).toFixed(2)}</p>
                  ) : (
                    <p className="text-xl font-medium text-yellow-900">Custom pricing</p>
                  )}
                  <p className="text-sm text-yellow-700 mt-2">Final quote will be sent within 24 hours</p>
                </div>
                ...
                <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  <ol className="space-y-2 text-sm text-gray-600">
                    <li className="flex items-start">
                      ...
                      You'll receive an email confirmation with a secure link to your quote
                    </li>
                    <li className="flex items-start">
                    ...
                    <li className="flex items-start">
                      ...
                      We'll send you a finalized quote that you can approve with one click
                    </li>
                    <li className="flex items-start">
                      <span className="font-bold text-blue-600 mr-2">4.</span>
                      Once approved, create an account to proceed with booking and payment
                    </li>
                  </ol>
                </div>
                ...
                <button
                  ...
                  className="w-full bg-green-600 text-white px-6 py-4 rounded-lg font-semibold text-lg hover:bg-green-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
                >
          ...
          <div className="hidden sm:flex mt-6 items-center justify-between">
            ...
            {currentStep < 7 && (
              ...
              >
                {currentStep === 3 ? 'Continue' : currentStep === 6 ? 'Review & Submit' : 'Next'}
              </button>
            )}
          </div>
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-40 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          {currentStep < 7 ? (
            ...
            >
              {currentStep === 3 ? 'Continue' : currentStep === 6 ? 'Review' : 'Next'}
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
